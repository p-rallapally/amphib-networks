...
- high connectance/density + low modularity

- same big dog amphibians (TATO/PSRE)
...
- perm ponds had richest parasite communities, semi-perm had densest interaction networks, and temp ponds had greatest modularity
...
- ID big dog parasites by year
- dynamic host breadth
- imbalance among parasite importance
...
- Preliminary work suggest that bullfrog presence is associated with changes in host–parasite network structure, while Bd burden appears to be driven more by host species than by pooled network centrality

to do: 

- incorporate wetland/survey details to contextualize differences in site-year networks
- linear mixed effect models(?) to predict richness, connectance, modularity 

- inc. rarification/standardization of bd 
- project bipartite network onto unipartite (jaccard similarity of host nodes), representation of transmission potential
...
- RACA has moderate sharing but highest prevalence
- TATO shares parasites w/ everybody yet lowest prevalence
- TATO has a giant bd load outlier that's messing with the data
- interconnected hosts = similar strengths
...
- significant diff among bd/prevalence in PSRE/RACA (no sig associations anywhere else)
...
bd_env_df <- bd_host_site_year %>%
  left_join(env_site, by = "site_code") %>%
...
- Semi-permanent ponds appear to have the highest median Bd prevalence, perm ponds are more bimodal, temp ponds are closer to 0 prevalence
- no diff among nutrient status
- generally decent correlation with pond depth/prevalence (strongest for RACA)
- No obvious reduction in Bd prevalence associated with drying events (big overlap)
- perm sites have higher parasite richness


```{r}

# STAGE 13: Rewiring - species turnover vs. interaction turnover,
# whole-network dissimilarity among taxa
#
# Follows Poisot et al. (2012)'s beta-diversity-of-interactions partition:
# beta_wn (whole network dissimilarity) decomposes into
#   beta_s  = dissimilarity from species turnover (who's present at all)
#   beta_os = dissimilarity from interaction rewiring among species
#             common to both networks (same players, different partners)

network_beta <- function(edges_a, edges_b) {

  links_a <- edges_a %>%
    distinct(spp_code, parasite_name) %>%
    mutate(link = paste(spp_code, parasite_name, sep = "__")) %>%
    pull(link)
  ...
  species_a <- unique(c(edges_a$spp_code, edges_a$parasite_name))
  species_b <- unique(c(edges_b$spp_code, edges_b$parasite_name))

  # whole-network dissimilarity: Jaccard over ALL realized interactions
  union_links <- length(union(links_a, links_b))
  shared_links <- length(intersect(links_a, links_b))
  beta_wn <- ifelse(union_links == 0, NA_real_, 1 - (shared_links / union_links))

  # species turnover: Jaccard over host + parasite taxa present in each network
  shared_species <- intersect(species_a, species_b)
  union_species <- union(species_a, species_b)
  beta_s <- ifelse(
    length(union_species) == 0,
    NA_real_,
    1 - (length(shared_species) / length(union_species))
  )

  # interaction rewiring: restrict to species common to BOTH networks, then
  # ask whether the interactions among those shared species changed
  edges_a_shared <- edges_a %>%
    filter(spp_code %in% shared_species, parasite_name %in% shared_species) %>%
    distinct(spp_code, parasite_name) %>%
    mutate(link = paste(spp_code, parasite_name, sep = "__")) %>%
    pull(link)

  edges_b_shared <- edges_b %>%
    filter(spp_code %in% shared_species, parasite_name %in% shared_species) %>%
    distinct(spp_code, parasite_name) %>%
    mutate(link = paste(spp_code, parasite_name, sep = "__")) %>%
    pull(link)

  shared_link_union <- union(edges_a_shared, edges_b_shared)
  shared_link_intersect <- intersect(edges_a_shared, edges_b_shared)

  beta_os <- ifelse(
    length(shared_link_union) == 0,
    NA_real_,
    1 - (length(shared_link_intersect) / length(shared_link_union))
  )
...
years_sorted <- hp_edges %>%
  distinct(year) %>%
  arrange(year) %>%
  pull(year)

# consecutive-year comparisons - matches the "mean jaccard increases over time" trend noted above, but decomposed into turnover vs. rewiring
yearly_rewiring <- map_dfr(
  seq_len(length(years_sorted) - 1),
  function(i){

    yr1 <- years_sorted[i]
    yr2 <- years_sorted[i + 1]

    edges_1 <- hp_edges %>% filter(year == yr1)
    edges_2 <- hp_edges %>% filter(year == yr2)

    network_beta(edges_1, edges_2) %>%
      mutate(year_from = yr1, year_to = yr2, .before = 1)
  }
)
...
yearly_rewiring %>%
  pivot_longer(
    cols = c(beta_wn, beta_s, beta_os),
    names_to = "beta_type",
    values_to = "value"
  ) %>%
  ggplot(aes(x = year_to, y = value, color = beta_type, group = beta_type)) +
  ...
  labs(
    ...
    y = "Dissimilarity",
    color = "Component",
    title = "Whole-network dissimilarity decomposed into turnover vs. rewiring",
    subtitle = "beta_wn = total | beta_s = species turnover | beta_os = interaction rewiring"
  ) +
  theme_minimal()

# all pairwise (not just consecutive) year comparisons, for the fuller picture
all_year_pairs <- combn(years_sorted, 2, simplify = FALSE)

pairwise_rewiring <- map_dfr(
  all_year_pairs,
  function(pair){

    edges_1 <- hp_edges %>% filter(year == pair[1])
    edges_2 <- hp_edges %>% filter(year == pair[2])

    network_beta(edges_1, edges_2) %>%
      mutate(year_from = pair[1], year_to = pair[2], .before = 1)
  }
)
...
to do: 
- [done] log transform bd before rarefaction
- [done] bd prevalence as a predictor of parasite outcomes
- [done, needs real wetland_info column names swapped in] env/pond properties as predictors of disease dynamics
- [done] (for rewiring) calc species turnover/interactions, whole network dissimilarity among taxa
- push to repo (add comments + clean org)
- ponder big picture outline/take home (defining impacts of two competing anthro effects)
  - brain dump on doc (intro + citations)
